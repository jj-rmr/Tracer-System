...
export async function GET(
  ...
  try {
    ...
    // 1. Fetch the primary account details
    const account = await getAccount(id);

    // 2. Safely attempt to pull the matching survey data
    let survey = null;
    ...
    try {
      ...
      // If the document isn't found or doesn't exist yet,
      // log it internally and let it remain null instead of crashing out.
      console.log(`No survey record found for account ID: ${id}`);
    }

    // 3. Return both together in a single round-trip response
    return NextResponse.json({
      success: true,
      account,
      survey,
