          ...
          {mainNavItems.map((item) => {
            ...
            return (
              <Link 
                key={item.href}
                href={item.href} 
                className={`flex flex-row items-center w-full h-fit pl-[19] py-4 rounded-2xl outline outline-sky-200 cursor-pointer hover:text-sky-400 hover:bg-sky-100 active:text-sky-400 active:bg-sky-50 active:scale-95 transition-all ease-out duration-300 ${isActiveLink(item.href) ? "bg-sky-100 text-sky-500" : ""}`}
              >
                <span className="shrink-0 inline-flex items-center justify-center">
                  <Icon size={24}/>
                </span>
          ...
          {secondaryNavItems.map((item) => {
            ...
            return (
              <Link 
                key={item.href}
                href={item.href} 
                className={`pl-5.5 py-2 flex flex-row items-center w-full h-fit cursor-pointer hover:text-sky-400 active:text-sky-400 active:scale-95 transition-all ease-out duration-300 ${isActiveLink(item.href) ? "text-sky-500" : ""}`}
              >
